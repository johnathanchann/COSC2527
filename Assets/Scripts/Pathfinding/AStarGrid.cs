...
public class AStarGrid : MonoBehaviour
{
    ...
    public LayerMask pondMask;

    public Vector2 gridWorldSize;
    public float overlapCircleRadius;
    public float gridSize;

    public float mudSpeedMultiplier;
    public float grassSpeedMultiplier;
    public float pondSpeedMultiplier;
    ...
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                bool mudArea = (Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, mudMask) != null);
                bool grassArea = (Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, grassMask) != null);
                bool pondArea = (Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, pondMask) != null);


                float speedMultiplier = 1f;
                if (mudArea)
                {
                    speedMultiplier = mudSpeedMultiplier;
                }
                else if (grassArea)
                {
                ...
                {
                    speedMultiplier = pondSpeedMultiplier;
                }
                grid[x, y] = new Node(walkable, mudArea, grassArea, pondArea, worldPoint, x, y, speedMultiplier);
    ...
    public void SetDynamicObstacle(Vector2 worldPos, bool isObstacle)
    {
        Node node = NodeFromWorldPoint(worldPos);
        if (node != null)
        {
            node.dynamicObstacle = isObstacle;
